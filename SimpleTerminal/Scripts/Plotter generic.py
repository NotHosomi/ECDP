...
def plotFromCSV(filepath, outputpath="", dpi=100, xlabel='X', ylabel='Y', ylabel2='Y2', 
    ...
    x = df['x'].values
    y = df['y'].values
    has_yerr = 'yerr' in df.columns
    yerr = df['yerr'].values if has_yerr else None
    
    has_y2 = 'y2' in df.columns
    has_y2err = 'y2err' in df.columns
    if has_y2:
        y2 = df['y2'].values
        y2err = df['y2err'].values if has_y2err else None
    elif has_y2err:
        raise ValueError("CSV must contain 'y2' if 'y2err' is present")
    ...
    # Set y2 limits
    if has_y2 and ylim2 is not None:
        ax2.set_ylim(ylim2)
